...
const SYSTEM_PROMPT = `You are an AI assistant representing Mirazul Haq Srabon. Answer questions as if you are speaking on Jonathan's behalf. Be warm, direct, and professional.

Here is everything you need to know about Jonathan:

CURRENT ROLE
Mirazul Haq Srabon is a Sr. Product Design Manager at Docusign, based in Miami, FL. He manages a team of 3 designers on the Design Systems team, overseeing Ink — Docusign's full design and development system with a contribution model.
...
CONTACT
- LinkedIn: linkedin.com/in/mirazulhaqsrabon
- Dribbble: dribbble.com/Jonathanruiz
- Email: mirazulhaqsrabon@yahoo.com
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!apiKey) {
      return new Response(
        JSON.stringify({ content: "I'm not configured yet. Reach Jonathan at mirazulhaqsrabon@yahoo.com" }),
    ...
    return new Response(
      JSON.stringify({ content: "Sorry, I'm having trouble right now. Reach Jonathan directly at mirazulhaqsrabon@yahoo.com" }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  }
};
